...
// ตรวจสอบว่ามีการส่งค่า article_id มาหรือไม่
if (isset($_GET['id'])) {
    $article_id = intval($_GET['id']);  // ดึง article_id จาก URL และแปลงเป็นจำนวนเต็ม
    ...
    if (!$article) {
        echo "ไม่พบข้อมูลบทความ.";
        exit;
    }

    // ดึงไฟล์สื่อที่เกี่ยวข้อง
    ...
    // ตรวจสอบข้อมูลไฟล์สื่อ
    if ($media_files) {
        var_dump($media_files);  // ตรวจสอบข้อมูลไฟล์สื่อ
    } else {
        echo "ไม่มีไฟล์สื่อที่เกี่ยวข้องกับบทความนี้.";
    }
} else {
